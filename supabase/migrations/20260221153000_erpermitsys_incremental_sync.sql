...
alter table public.erpermitsys_contacts
    add column if not exists updated_by text not null default '',
    add column if not exists deleted_at timestamptz;

alter table public.erpermitsys_jurisdictions
    add column if not exists updated_by text not null default '',
    add column if not exists deleted_at timestamptz;

alter table public.erpermitsys_properties
    add column if not exists updated_by text not null default '',
    add column if not exists deleted_at timestamptz;

alter table public.erpermitsys_permits
    add column if not exists updated_by text not null default '',
    add column if not exists deleted_at timestamptz;

alter table public.erpermitsys_document_templates
    add column if not exists updated_by text not null default '',
    add column if not exists deleted_at timestamptz;

alter table public.erpermitsys_active_document_templates
    add column if not exists updated_by text not null default '',
    add column if not exists deleted_at timestamptz;
...
create index if not exists eps_contacts_active_idx
    on public.erpermitsys_contacts (app_id, updated_at desc)
    where deleted_at is null;

create index if not exists eps_jurisdictions_active_idx
    on public.erpermitsys_jurisdictions (app_id, updated_at desc)
    where deleted_at is null;

create index if not exists eps_properties_active_idx
    on public.erpermitsys_properties (app_id, updated_at desc)
    where deleted_at is null;

create index if not exists eps_permits_active_idx
    on public.erpermitsys_permits (app_id, updated_at desc)
    where deleted_at is null;

create index if not exists eps_templates_active_idx
    on public.erpermitsys_document_templates (app_id, updated_at desc)
    where deleted_at is null;

create index if not exists eps_active_templates_active_idx
    on public.erpermitsys_active_document_templates (app_id, updated_at desc)
    where deleted_at is null;

create or replace function public.erpermitsys_fetch_snapshot(
    p_app_id text default 'erpermitsys'
) returns jsonb
language plpgsql
as $$
declare
        ...
        'payload', jsonb_build_object(
            ...
            coalesce(
                (
                    ...
                    from public.erpermitsys_contacts
                    where app_id = v_app_id
            ...
            coalesce(
                (
                    ...
                    from public.erpermitsys_jurisdictions
                    where app_id = v_app_id
                      and deleted_at is null
                ),
                '[]'::jsonb
            ...
            coalesce(
                (
                    ...
                    from public.erpermitsys_properties
                    where app_id = v_app_id
                      and deleted_at is null
                ),
                '[]'::jsonb
            ...
            coalesce(
                (
                    ...
                    from public.erpermitsys_permits
                    where app_id = v_app_id
                      and deleted_at is null
                ),
                '[]'::jsonb
            ...
            coalesce(
                (
                    ...
                    from public.erpermitsys_document_templates
                    where app_id = v_app_id
                      and deleted_at is null
                ),
                '[]'::jsonb
            ...
            coalesce(
                (
                    select jsonb_object_agg(permit_type, template_id)
                    from (
                        select permit_type, template_id
                        from public.erpermitsys_active_document_templates
                        where app_id = v_app_id
                          and deleted_at is null
...
begin
    ...
    set
        ...
        payload = coalesce(public.erpermitsys_fetch_snapshot(v_app_id) -> 'payload', '{}'::jsonb)
    where app_id = v_app_id;
...
grant execute on function public.erpermitsys_fetch_snapshot(text) to public;
